...
static DWORD CpuExecuteKnownOP(void)
{
    DWORD cycle = 0;
//
...
#define OPCODE_HANDLER(hi, lo) &CpuExecuteKnownOP<0x##hi##lo>
#define OPCODE_ROW(hi) \
    OPCODE_HANDLER(hi,0), OPCODE_HANDLER(hi,1), \
    OPCODE_HANDLER(hi,2), OPCODE_HANDLER(hi,3), \
    OPCODE_HANDLER(hi,4), OPCODE_HANDLER(hi,5), \
    OPCODE_HANDLER(hi,6), OPCODE_HANDLER(hi,7), \
    OPCODE_HANDLER(hi,8), OPCODE_HANDLER(hi,9), \
    OPCODE_HANDLER(hi,A), OPCODE_HANDLER(hi,B), \
    OPCODE_HANDLER(hi,C), OPCODE_HANDLER(hi,D), \
    OPCODE_HANDLER(hi,E), OPCODE_HANDLER(hi,F)

static CpuOpcodeHandler const opcode_handlers[256] = {
    OPCODE_ROW(0), OPCODE_ROW(1), OPCODE_ROW(2), OPCODE_ROW(3),
    OPCODE_ROW(4), OPCODE_ROW(5), OPCODE_ROW(6), OPCODE_ROW(7),
    OPCODE_ROW(8), OPCODE_ROW(9), OPCODE_ROW(A), OPCODE_ROW(B),
    OPCODE_ROW(C), OPCODE_ROW(D), OPCODE_ROW(E), OPCODE_ROW(F)
};

#undef OPCODE_ROW
#undef OPCODE_HANDLER

static const BYTE opcode_lengths[256] = {
    ...
    2,2,2,1,1,2,2,1,1,3,1,1,1,3,3,1,
    1,2,1,1,2,2,2,1,1,2,1,1,3,3,3,1,
    2,2,2,1,2,2,2,1,1,3,1,1,3,3,3,1,
...
CpuOpcodeHandler CpuGetOpcodeHandler(BYTE opcode)
{
    return opcode_handlers[opcode];
}

BYTE CpuGetOpcodeLength(BYTE opcode)
{
    return illegal_op_byte[opcode] ? illegal_op_byte[opcode]
                                   : opcode_lengths[opcode];
}

DWORD CpuExecuteOP(void)
{
    return opcode_handlers[CPU_PEEK((uint16_t)mPC)]();
}
