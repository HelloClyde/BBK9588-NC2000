...
Dsp::Dsp() {
    ...
    dspCelpOff=0;
    callback=0;
    word_collected_samples=0;
    word_frame_read=0;
    word_sample_cursor=0;
    word_start=word_end=0;
    word_job_active=false;
}
...
void Dsp::write(int high,int low) {
	...
	if (high<0x60) {//is celp data
		...
		if(low==WORD_MODE && dspMode!=WORD_MODE && !word_job_active){
			word_frames.clear();
			word_collected_samples=0;
		}
        dspMode=low;
        ...
        if(high==0xc2){//音节起始offset
            ...
            int end=end3;
            if(enable_dsp_verbose_log){
                printf("[DSP] c2=%d c4=%d end1=%d end2=%d word_samples=%d c8=%d\n",c2, c4,end1,end2,(int)word_collected_samples,c8);
            }
            if(end>(int)word_collected_samples){
                if(enable_dsp_log) printf("[DSP] oops, end=%d word_samples=%d, out of range\n",end,(int)word_collected_samples);
                end=(int)word_collected_samples;
            }
            word_frame_read=0;
            word_sample_cursor=0;
            word_start=c2<(int)word_collected_samples ?
                       (unsigned int)c2 : word_collected_samples;
            word_end=end>0 ? (unsigned int)end : 0u;
            word_job_active=word_frames.size()>0 && word_start<word_end;
            data_cnt=0;
            if(!word_job_active) finishWordJob();
        }else{
...
void Dsp::dspCelpToCelp() {
    ...
    if(dspMode==WORD_MODE){
        if(word_frames.size()<256u){
            WordCelpFrame frame;
            for(int i=0;i<18;i++) frame.data[i]=clpBuf[i];
            frame.sample_count=(unsigned short)len;
            word_frames.push_back(frame);
            word_collected_samples+=(unsigned int)len;
        }
        return;
    }
    dsp(clpBuf);
    callback((unsigned char *)Sout,len*(int)sizeof(Sout[0]));
}

void Dsp::finishWordJob() {
    word_job_active=false;
    word_frames.clear();
    word_collected_samples=0;
    word_frame_read=0;
    word_sample_cursor=0;
    word_start=word_end=0;
    // WORD mode resets the synthesis history after each syllable/word.
    dspStart();
}

void Dsp::process_pending_word(int max_frames) {
    while(word_job_active && max_frames-->0){
        if(word_frame_read>=word_frames.size() ||
           word_sample_cursor>=word_end){
            finishWordJob();
            break;
        }

        WordCelpFrame &frame=word_frames[word_frame_read++];
        unsigned int frame_start=word_sample_cursor;
        unsigned int frame_end=frame_start+frame.sample_count;
        dsp(frame.data);

        unsigned int emit_start=frame_start>word_start ? frame_start : word_start;
        unsigned int emit_end=frame_end<word_end ? frame_end : word_end;
        if(emit_start<emit_end){
            unsigned int local_start=emit_start-frame_start;
            unsigned int local_end=emit_end-frame_start;
            for(unsigned int local=local_start;local<local_end;local++){
                unsigned int position=frame_start+local;
                int value=Sout[local];
                unsigned int from_start=position-word_start;
                unsigned int from_end=word_end-1u-position;
                if(from_start<(unsigned int)fade_range)
                    value=value*(int)from_start/fade_range;
                if(from_end<(unsigned int)fade_range)
                    value=value*(int)from_end/fade_range;
                Sout[local]=(short)value;
            }
            callback((unsigned char *)&Sout[local_start],
                     (int)((local_end-local_start)*sizeof(Sout[0])));
        }
        word_sample_cursor=frame_end;
        if(word_sample_cursor>=word_end || word_frame_read>=word_frames.size())
            finishWordJob();
    }
}

unsigned int Dsp::buffered_samples() const {
    if(!word_job_active) return word_collected_samples;
    return word_end>word_sample_cursor ?
           word_end-word_sample_cursor : 0u;
}
